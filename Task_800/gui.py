...
from work_with_files import dbase
from work_with_files import id_functions as idf

def click_save():
    user_id = idf.id_generator(file_path, file_format.get())
    result_str = [user_id,
                ent_fname_input.get(),
                ent_lname_input.get(),
...
def click_find():
    ...
    if result != None:
        ...
        lbl_user_id_out.configure(text = result_of_find[0])
        ent_fname_input.delete('0', tk.END)
        ent_fname_input.insert('0', result_of_find[1])
        ent_lname_input.delete('0', tk.END)
        ent_lname_input.insert('0', result_of_find[2])
        ent_phone_1_input.delete('0', tk.END)
        ent_phone_1_input.insert('0', result_of_find[3])
        ent_phone_2_input.delete('0', tk.END)
        ent_phone_2_input.insert('0', result_of_find[4])
        lbl_fresult.configure(text = 'Запись найдена')
    else:
        lbl_fresult.configure(text = 'Запись не найдена')

def click_clear():
    lbl_user_id_out.configure(text = 'ID записи')
    ent_fname_input.delete('0', tk.END)
    ent_lname_input.delete('0', tk.END)
    ent_phone_1_input.delete('0', tk.END)
    ent_phone_2_input.delete('0', tk.END)
...
def click_del():
    result_str = [lbl_user_id_out.cget('text'),
                ent_fname_input.get(),
                ent_lname_input.get(),
                ent_phone_1_input.get(),
                ent_phone_2_input.get()]
...
window = tk.Tk()
window.title('Телефонный справочник 0.5')
file_path = Path.cwd()
...
# Внесение ID
lbl_user_id = tk.Label(master = frm_form, text = 'ID', font = ('Calibri', 16))
lbl_user_id_out = tk.Label(master = frm_form, text = 'ID записи', font = ('Calibri', 12))
lbl_user_id.grid(row = 0, column = 0, sticky = 'e')
lbl_user_id_out.grid(row = 0, column = 1, sticky = 'w')
